...
@Component({
  selector: 'app-d-completed-orders',
  templateUrl: './d-completed-orders.component.html',
  providers:[DatePipe],
  styleUrls: ['./d-completed-orders.component.css']
})
export class DCompletedOrdersComponent implements OnInit {
  type: any;
  city_code=[];
  filterType: number=0;
  errorMessage: string='';
  ...
  changed(event){
    ...
    if(this.totalLength > this.c_order.length)
    this.totalLength= this.c_order.length;
  }
  FilterData(){
  ...
  fetchedDashboardData(res){
    ...
    this.dataLoaded=1;
    if(res.success == true){
      this.c_order = res.completed_orders;
    }else{
      this.c_order = [];
    }
