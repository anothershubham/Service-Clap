...
@Component({
  selector: 'app-d-partners',
  templateUrl: './d-partners.component.html',
  providers:[DatePipe],
  styleUrls: ['./d-partners.component.css']
})
export class DPartnersComponent implements OnInit {
  type: any;
  loading='0';
  city_code=[];
  dataLoaded:number=0;
  ...
  changed(event){
    ...
    if(this.totalLength > this.partners.length)
  ...
  fetchedDashboardData(res){
    ...
    this.dataLoaded=1;
    if(res.success == true){
      this.partners = res.total_partner;
      this.totalLength = this.currentPage * this.itemsPerPage;
      if(this.totalLength > this.partners.length)
      this.totalLength= this.partners.length;
    }else{
